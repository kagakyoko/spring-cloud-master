...
public class HttpResponseProxy {
    ...
    private String getFileName(CloseableHttpResponse response) throws HttpErrorException {
        ...
        if (contentDispositionHeader == null || contentDispositionHeader.length == 0) {
            throw new HttpErrorException(HttpError.builder().errorMsg("无法获取到文件名").build());
        }

        return this.extractFileNameFromContentString(contentDispositionHeader[0].getValue());
    }

    private String getFileName(HttpResponse response) throws HttpErrorException {
        String content = response.header("Content-disposition");
        return this.extractFileNameFromContentString(content);
    }

    private String getFileName(Response response) throws HttpErrorException {
        String content = response.header("Content-disposition");
        return this.extractFileNameFromContentString(content);
    }
